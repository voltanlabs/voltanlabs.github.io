...
(function(){
  ...
  function render(){
    const renderStage=location.pathname.includes('runtime-diagnostic')
      ?new URLSearchParams(location.search).get('renderStage')
      :null;
    installShellStyle();
    if(renderStage==='style')return;
    ensureRoot();
    if(renderStage==='root')return;
    const ctx=screenContext();
    if(renderStage==='context')return;
    const nextScreen=screenHtml(ctx);
    const nextControls=controlsHtml(ctx);
    if(renderStage==='markup')return;
    $('stage').innerHTML=nextScreen;
    $('controls').innerHTML=nextControls;
    if(renderStage==='commit')return;
    applyControlHost();
    bind();
    if(renderStage==='bind')return;
    paintFx();
  }
  ...
  const diagnosticShellStage=location.pathname.includes('runtime-diagnostic')
    ?new URLSearchParams(location.search).get('shellStage')
    :null;

  if(diagnosticShellStage!=='definition')seed();
  if(diagnosticShellStage!=='definition'&&diagnosticShellStage!=='seed')render();
