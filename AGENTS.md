...
The active milestone is **Data Discovery Phase 6.3.0 Portal Battle Presentation**, loaded by bootstrap 1.11.0 as one generated bundle containing 19 explicitly ordered browser modules. The live shell remains v4.11.0. Phase 6.3 owns the portal canvas, synchronized rim motion, depth gradient, rarity accents, responsive mobile framing, and stage-background fallback selection. Older documents and legacy files may describe v3, v3.5, or the Phase 4 compatibility chain; do not assume that the newest-looking filename or document is active. The live HTML entry point, bootstrap registry, runtime manifest, ownership map, and exported browser globals must agree before behavior changes are merged.
