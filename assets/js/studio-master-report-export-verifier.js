...
(function(){
  const VERSION='1.0.0';
  function build(){return window.VoltanMasterReportBridge&&window.VoltanMasterReportBridge.build?window.VoltanMasterReportBridge.build():null}
  function verify(){
    const report=build();
    ...
    result.ok=Object.keys(result.checks).every(k=>!!result.checks[k]);
    window.VOLTAN_MASTER_REPORT_EXPORT_VERIFICATION=result;
    document.dispatchEvent(new CustomEvent('studio:master-report-export-verified',{detail:result}));
    return result;
  }
  function render(){
    ...
    box.insertAdjacentHTML('afterbegin',`<section id="studioMasterReportExportVerifier" class="lg:col-span-2 rounded-2xl border border-emerald-400/40 bg-[#06180f]/80 p-5"><div class="flex flex-wrap items-start justify-between gap-4"><div><p class="text-xs uppercase tracking-wide text-[#FFD700]">Studio Intelligence</p><h2 class="text-2xl font-bold text-[#FFD700] mt-1">Master Report Export Verification</h2><p class="text-gray-300 mt-3">Validates that Copy and Save are exporting the enriched Master Studio Report payload.</p></div><div class="rounded-xl border border-white/10 bg-black/25 p-4 min-w-[150px]"><p class="text-gray-400 text-sm">Export Status</p><strong class="text-2xl ${r.ok?'text-emerald-200':'text-yellow-200'}">${r.ok?'Ready':'Check'}</strong></div></div><div class="overflow-x-auto mt-4"><table class="w-full text-sm text-left"><thead class="text-gray-300"><tr><th class="py-2 pr-3">Check</th><th class="py-2">Present</th></tr></thead><tbody>${rows}</tbody></table></div></section>`);
  }
  function boot(){setTimeout(()=>{verify();render()},1700);document.addEventListener('studio:diagnostics-ready',()=>setTimeout(()=>{verify();render()},300));document.addEventListener('studio:intelligence-collected',()=>setTimeout(verify,50))}
