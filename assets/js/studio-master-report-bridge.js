...
(function(){
  ...
  function manager(){return window.VOLTAN_STUDIO_INTELLIGENCE||null}
  ...
  function fallbackReport(){return {generatedAt:new Date().toISOString(),engineVersion:'fallback-export',sources:[],findings:[],exportWarning:'Validation report was not ready when export was requested.'}}
  function build(){
    ...
    source.studioIntelligence=safe(()=>collectIntelligence(),{version:VERSION,error:'Studio Intelligence read failed but export continued.'});
    return source;
  }
  ...
  async function copy(btn){const payload=build();try{await navigator.clipboard.writeText(JSON.stringify(payload,null,2));setButton(btn,'Master Report Copied')}catch(e){setButton(btn,'Copy Failed')}return payload}
  function save(btn){try{const payload=build();const stamp=new Date().toISOString().replace(/[:.]/g,'-');download(`voltanlabs-master-report-${stamp}.json`,payload);window.VOLTAN_LAST_MASTER_REPORT_EXPORT=payload;setButton(btn,'Download Started');return payload}catch(e){console.error('Master Report save failed',e);setButton(btn,'Save Failed');return null}}
  function bind(){
  ...
  function refreshPreview(){window.VOLTAN_MASTER_REPORT_PREVIEW=safe(()=>build(),null)}
  function boot(){bind();let n=0;const t=setInterval(()=>{bind();if(++n>80)clearInterval(t)},250);document.addEventListener('studio:diagnostics-ready',()=>setTimeout(refreshPreview,50));setTimeout(refreshPreview,1800)}
