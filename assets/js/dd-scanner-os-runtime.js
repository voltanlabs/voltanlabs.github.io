...
(function(){
  ...
  function addStyle(){
    ...
    s.textContent=[
      ...
      '#ddApp .battle-card>.hint{grid-row:4!important}',
      '#ddApp .battle-card>.battleLog{grid-row:5!important}',
  ...
  function cleanupBattleUi(){
    ...
    if(battleCard){
      Array.prototype.slice.call(battleCard.children).forEach(function(node){
        ...
        if(node.classList&&node.classList.contains('hint'))return;
  ...
  function tag(){
    var app=document.getElementById('ddApp');
    if(app)app.dataset.scannerOsRuntime='canonical-4-3-battle-cleanup';
  }
  ...
  function boot(){
    ...
    document.dispatchEvent(new CustomEvent('dd:scanner-os-runtime-ready',{detail:{id:'dd-scanner-os-runtime',phase:'4.3',battleUi:'cleaned'}}));
  }
