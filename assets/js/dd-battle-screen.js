...
(function(){
  ...
  function installStyle(){
    ...
    style.textContent=[
      ...
      '#ddApp .battle-card[data-owner="dd-battle-screen"] .ring{width:min(31vw,168px);height:min(31vw,168px);border-radius:999px;margin:0 auto;display:grid;place-items:center;background:radial-gradient(circle,rgba(7,17,31,.9) 0 57%,rgba(15,23,42,.56) 58% 72%,transparent 73%);border:0;position:relative;transform:none!important;box-sizing:border-box;transition:filter .18s ease}',
      '#ddApp .battle-card[data-owner="dd-battle-screen"] .ring::before{content:"";position:absolute;inset:2px;border-radius:inherit;background:conic-gradient(from -90deg,var(--hp-color) 0 calc(var(--hp-pct)*1%),rgba(71,85,105,.42) calc(var(--hp-pct)*1%) 100%);-webkit-mask:radial-gradient(farthest-side,transparent calc(100% - 7px),#000 calc(100% - 6px));mask:radial-gradient(farthest-side,transparent calc(100% - 7px),#000 calc(100% - 6px));animation:ddPortalSpin 7s linear infinite;filter:drop-shadow(0 0 8px color-mix(in srgb,var(--hp-color) 58%,transparent));z-index:0}',
      '#ddApp .battle-card[data-owner="dd-battle-screen"] .ring::after{content:"";position:absolute;inset:13%;border:1px solid color-mix(in srgb,var(--hp-color) 42%,transparent);border-radius:inherit;box-shadow:inset 0 0 22px color-mix(in srgb,var(--hp-color) 20%,transparent),0 0 18px rgba(56,189,248,.16);animation:ddPortalPulse 2.4s ease-in-out infinite;z-index:0}',
      '#ddApp .battle-card[data-owner="dd-battle-screen"] .avatar{position:relative;width:100%;height:100%;border-radius:999px;display:grid;place-items:center;background:transparent;font-size:clamp(28px,8vw,40px);line-height:1;z-index:1;overflow:visible}',
