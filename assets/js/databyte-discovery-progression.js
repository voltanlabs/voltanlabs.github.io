...
(function () {
  ...
  function familyProgress(group) {
    const stages = group.map((index, stageIndex) => {
      ...
      return { name, count, inventory, decompiled, unlocked, unlocksNext: Boolean(nextName), nextName, stageIndex };
    });
  ...
  function allFamilyProgress() {
    return baseGroups.filter((group) => group.length > 1).map(familyProgress);
  }

  function applyProgression() {
    window.DD_ENCOUNTER_GROUPS = baseGroups.map(unlockedGroup);
    window.DD_STAGE_WEIGHTS = { one: [100], two: [92, 8], three: [88, 10, 2] };
  ...
  function signature() {
    return JSON.stringify({
      collection: read(COLLECTION_KEY).map((sprite) => sprite?.name || "").join("|"),
      decompiled: read(DECOMPILE_KEY).map((sprite) => sprite?.name || "").join("|")
    });
  }

  function renderProgressionBadge(force = false) {
    applyProgression();
    addProgressionPanel();
    ...
    panel.innerHTML = `
      ...
      <div id="evolutionProgressList" class="mt-3 grid gap-2 max-h-72 overflow-auto pr-1">
        ${families.map((family) => {
          ...
          return `<div class="bg-black/20 rounded-xl p-3 border border-white/10"><div class="flex justify-between gap-3"><strong>${family.root}</strong><span class="text-[#FFD700] text-xs">${Math.min(goal.count, goal.required)}/${goal.required}</span></div><p class="text-gray-300 text-xs mt-1">Capture <strong>${goal.priorName}</strong> to unlock <strong>${goal.targetName}</strong>.</p><div class="text-[#FFD700] tracking-widest mt-2">${progressBar(goal.count)}</div><div class="text-[10px] text-gray-400 mt-1">Inventory ${goal.inventory} • Decompiled ${goal.decompiled}</div></div>`;
        }).join("")}
      </div>`;
  ...
  function scheduleRender() {
    requestAnimationFrame(() => renderProgressionBadge());
  }

  function bootProgression() {
    applyProgression();
    renderProgressionBadge(true);
  ...
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", bootProgression);
  else bootProgression();
})();