...
// Phase 3.3: canonical battle resolution with configuration and status-effect outcomes.
(function () {
  'use strict';
  ...
  function battle() {
    return window.DDBattle24 || null;
  }

  function rules() {
    return window.DD_GAMEPLAY_RULES || null;
  }
  ...
  function normalizeStatusEffect(effect) {
    ...
    return {
      ...
      chance: clamp(
        effect.chance === undefined ? 100 : effect.chance,
        0,
        100
      ),
      durationTurns: clamp(
        effect.durationTurns || effect.duration || 1,
        1,
        99
      ),
      ...
      target:
        ...
        null,
      data:
        effect.data && typeof effect.data === 'object'
  ...
  function normalizeMove(move) {
    ...
    if (
      rules() &&
      typeof rules().tuneMove === 'function'
    ) {
      move = rules().tuneMove(move);
    }

    move.power = clamp(move.power || 24, 0, 60);
    move.accuracy = clamp(move.accuracy || 90, 0, 100);
    move.captureEffect = clamp(
      move.captureEffect !== undefined
        ? move.captureEffect
        : move.downloadEffect || 1,
      0,
      8
    );
  ...
  function typeResult(move, target) {
    if (
      battle() &&
      typeof battle().typeResult === 'function'
    ) {
      return battle().typeResult(move, target);
    }
  ...
  function hitCheck(user, move, target, seed) {
    ...
    return {
      hit: roll < Number(move.accuracy || 90),
      roll: roll,
      accuracy: Number(move.accuracy || 90)
    };
  }
  ...
  function resolveStatus(user, move, target, opts, hit) {
    ...
    return {
      ...
      target: appliesTo,
      targetId:
        recipient && (recipient.id || recipient.name) || null,
      data: Object.assign(
        {
  ...
  function resolve(user, move, target, opts) {
    ...
    var hit = hitCheck(
      ...
      move,
      target,
      opts.seed || opts.mode || 'turn'
    );
    var type = typeResult(move, target);
    var effectiveHit =
      hit.hit &&
      Number(type.multiplier) !== 0;
    ...
    if (!effectiveHit) {
      return {
        ...
        blockedByConfiguration:
          hit.hit && Number(type.multiplier) === 0,
        type: type,
        move: move,
        hpDamage: 0,
        signalDamage: 0,
        ...
        notes: [
          ...
          ' used ' +
          move.name +
          (hit.hit ? ', but it had no effect.' : ', but missed.')
        ]
      };
    }
    ...
    var sameConfiguration =
      sameConfigurationBonus(user, move);
  ...
  function chooseEnemyMove(enemy, lead) {
    ...
    if (
      battle() &&
      typeof battle().chooseEnemyMove === 'function'
    ) {
      var picked =
        battle().chooseEnemyMove(
          enemy,
          lead
        );
    ...
    var sorted = moves.slice().sort(
      function (left, right) {
        return Number(right.power || 0) -
          Number(left.power || 0);
      }
    );
    ...
    return value % 100 < 70
      ? sorted[0]
      : moves[value % moves.length];
  }

  function turnOrder(lead, wild) {
    var leadSpeed =
      Number(lead && lead.speed || 8);
    var wildSpeed =
      Number(wild && wild.speed || 8);
