(function(){
  ...
  if(!session)return;
  const slots=session.slots(),occupied=new Set(slots.filter(Boolean).map(item=>item.id)),repo=session.repository();
  const clean=repo.filter(item=>item&&!occupied.has(item.id));
