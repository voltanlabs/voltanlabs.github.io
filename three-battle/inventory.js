(function () {
  ...
  function render() {
    ...
    list.querySelectorAll('[data-item-id]').forEach(button => button.onclick = () => { const id = button.dataset.itemId; const action = id === 'patch' ? 'healPlayer' : id; const inBattle = !document.getElementById('arenaView')?.classList.contains('hidden'); if (!inBattle) { if (id === 'boost') return; const active = window.DataByteSession?.starter?.(); const item = window.DataByteSession?.party?.().find(entry => (entry.uid || entry.id) === active || entry.id === active); if (!item) return; if (!use(id)) return; const amount = id === 'patch' ? 25 : 35; window.DataByteSession.updateHp(item.uid || item.id, Math.min(item.maxHp ?? 100, Number(item.hp ?? 100) + amount)); render(); return; } const battle = window.DataByteBattle, battleState = battle?.getState?.(); if (!battle?.[action] || battleState?.busy || battleState?.over) return; if (!count(id)) { battle.message(`No ${button.querySelector('strong').textContent} items remaining.`); return; } if (!battle[action](id === 'patch' ? 25 : undefined)) return; use(id); });
