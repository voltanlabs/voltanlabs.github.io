(function () {
  ...
  session.party = party; session.repository = repository; session.slots = slots; session.starter = starter; session.setStarter = setStarter; session.setLead = setLead; session.updateHp = updateHp; session.addSpriteXp = addSpriteXp; session.capture = capture; session.store = store; session.storeSlot = storeSlot; session.deploy = deploy; session.swapSlots = swapSlots; session.assignSlot = assignSlot; session.deleteInstance = deleteInstance; session.importSave = importSave; session.identity = uid; session.__instanceStorage = true;
})();
