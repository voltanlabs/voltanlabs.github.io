(function () {
  ...
  const normalizeInstance = item => { const next = { ...item }; next.captureHp = Math.max(1, Math.min(hpCap(next), Number(next.captureHp) || Math.min(100, Number(next.maxHp) || 100))); next.maxHp = progressionFor(next).maxHp; next.hp = Math.min(next.maxHp, Math.max(0, Number(next.hp ?? next.maxHp))); next.statRolls = statRollsFor(next); next.stats = statsFor(next); next.statsVersion = 3; return next; };
