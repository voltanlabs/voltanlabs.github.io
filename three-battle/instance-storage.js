(function () {
  ...
  const createWildInstance = source => { const level = Math.max(1, Math.min(100, Number(source?.level) || 1)), captureHp = 60 + Math.floor(Math.random() * 41), wild = normalizeInstance({ ...source, uid: `wild-${source?.id || 'signal'}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, captureHp, maxHp: captureHp, hp: captureHp, level }); wild.hp = wild.maxHp; return wild; };
