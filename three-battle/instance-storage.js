(function () {
  ...
  // Temporary ceiling for wild stat generation only. Owned instances may grow beyond it.
  const WILD_STAT_CEILING = 160;
  ...
  const HP_LEVEL_GROWTH = 0.85, HP_FORM_BONUS = 50;
  const captureHpRange = item => ({ 1: [60, 100], 2: [90, 140], 3: [120, 180] }[stageFor(item)] || [60, 100]);
  const levelFor = item => Math.max(1, Math.min(100, Number(item?.level) || 1));
  const hpFor = item => { const base = Math.max(1, Number(item?.captureHp) || 60), levelGrowth = Math.round((levelFor(item) - 1) * HP_LEVEL_GROWTH), formBonus = Math.max(0, stageFor(item) - 1) * HP_FORM_BONUS; return Math.max(1, Math.round(base + levelGrowth + formBonus)); };
  ...
  const statsFor = item => { const rolls = statRollsFor(item), stageBonus = (stageFor(item) - 1) * 18, growth = (levelFor(item) - 1) * 0.85, wildCap = item?.wildInstance === true ? value => Math.min(WILD_STAT_CEILING, value) : value => value; return { attack: Math.max(1, wildCap(Math.round(rolls.attack + stageBonus + growth))), defense: Math.max(1, wildCap(Math.round(rolls.defense + stageBonus + growth))), speed: Math.max(1, wildCap(Math.round(rolls.speed + stageBonus + growth))), crit: Math.max(0, Math.min(50, Number(item?.stats?.crit ?? item?.crit ?? Math.floor(Math.random() * 51)))) }; };
  const progressionFor = item => { const current = Math.max(1, Number(item?.captureHp) || Math.min(100, Number(item?.maxHp) || 100)), maxHp = hpFor({ ...item, captureHp: current }); return { maxHp, hp: Math.min(maxHp, Math.max(0, Number(item?.hp ?? maxHp))) }; };
  const createWildInstance = source => { const level = Math.max(1, Math.min(100, Number(source?.level) || 1)), [minimumHp, maximumHp] = captureHpRange(source), captureHp = minimumHp + Math.floor(Math.random() * (maximumHp - minimumHp + 1)), xp = Math.max(Number(source?.xp) || 0, 50 * (level - 1) * level), wild = normalizeInstance({ ...source, uid: `wild-${source?.id || 'signal'}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, captureHp, maxHp: captureHp, hp: captureHp, level, xp, wildInstance: true }); wild.hp = wild.maxHp; return wild; };
  const normalizeInstance = item => { const next = { ...item }; next.xp = Math.max(0, Number(next.xp) || 0); next.level = session.spriteProgress?.(next)?.level || 1; next.captureHp = Math.max(1, Number(next.captureHp) || Math.min(100, Number(next.maxHp) || 100)); next.maxHp = progressionFor(next).maxHp; next.hp = Math.min(next.maxHp, Math.max(0, Number(next.hp ?? next.maxHp))); next.statRolls = statRollsFor(next); next.stats = statsFor(next); next.statsVersion = 3; return next; };
  ...
  function deleteInstance(id) { const active = slots(), stored = repository(), key = id, exists = [...active, ...stored].some(item => item && (uid(item) === key || item.id === id)); if (!exists) return false; const wasLead = starter() === key, nextActive = active.map(item => item && (uid(item) === key || item.id === id) ? null : item), nextStored = stored.filter(item => !(uid(item) === key || item.id === id)); save(nextActive, nextStored); if (wasLead) { const replacement = nextActive.find(item => item && Number(item.hp ?? 100) > 0); if (replacement) setStarter(uid(replacement)); else localStorage.removeItem('vl_three_battle_starter'); } window.dispatchEvent(new CustomEvent('databyte:party-updated')); return true; }
  function importSave(payload) { let data = payload; try { if (typeof payload === 'string') data = JSON.parse(payload); } catch { return { ok: false, reason: 'invalid-json' }; } if (!data || !Array.isArray(data.party) || !Array.isArray(data.repository)) return { ok: false, reason: 'invalid-save' }; const normalize = (list, prefix) => { const used = new Set(); return list.filter(Boolean).map((item, index) => { const next = { ...item, uid: item.uid || `${item.id || 'databyte'}-${prefix}-${index + 1}` }; while (used.has(next.uid)) next.uid = makeUid(next.id); used.add(next.uid); return normalizeInstance(next); }); }; const active = normalize(data.party, 'import-party').slice(0, 5), stored = normalize(data.repository, 'import-repo').filter(item => !active.some(entry => uid(entry) === uid(item))); save(active, stored); const requested = data.starter || ''; const lead = active.find(item => uid(item) === requested || item.id === requested); if (lead && Number(lead.hp ?? 100) > 0) localStorage.setItem('vl_three_battle_starter', uid(lead)); else localStorage.removeItem('vl_three_battle_starter'); window.dispatchEvent(new CustomEvent('databyte:party-updated')); window.dispatchEvent(new CustomEvent('databyte:dex-updated')); return { ok: true }; }
    session.party = party; session.repository = repository; session.slots = slots; session.starter = starter; session.setStarter = setStarter; session.setLead = setLead; session.updateHp = updateHp; session.addSpriteXp = addSpriteXp; session.capture = capture; session.store = store; session.storeSlot = storeSlot; session.deploy = deploy; session.swapSlots = swapSlots; session.assignSlot = assignSlot; session.deleteInstance = deleteInstance; session.importSave = importSave; session.identity = uid; session.createInstanceStats = statsFor; session.createWildInstance = createWildInstance; session.refreshInstance = normalizeInstance; session.__instanceStorage = true;
})();
