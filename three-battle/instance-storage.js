(function () {
  ...
  const statsFor = item => { const configuration = item?.primaryConfiguration || item?.configurations?.[0] || item?.configuration || 'Default', base = statBases[configuration] || statBases.Default, stage = window.DataByteProgressionData?.stageFor?.(item) || 1, stageBonus = [-8, 0, 8][stage - 1] || 0, text = `${item?.id || ''}:${item?.uid || ''}`; let seed = 0; for (const char of text) seed = (seed * 31 + char.charCodeAt(0)) >>> 0; const variation = value => Math.max(1, Math.min(99, value + stageBonus + (seed % 13) - 6)); return { attack: variation(base[0]), defense: variation(base[1]), speed: variation(base[2]), crit: Math.max(1, 5 + stageBonus / 2 + (seed % 11)) }; };
  const read = (key, prefix) => {
    ...
    list = list.filter(Boolean).map((item, index) => {
      ...
      if (!next.stats || next.statsVersion !== 2) { next.stats = statsFor(next); next.statsVersion = 2; changed = true; }
  ...
  function setStarter(id) {
    ...
    const added = { ...source, uid: makeUid(source.id), sprite: './data/sprites/' + (source.sprite || 'placeholder.png'), hp: 100, maxHp: 100, xp: 0, level: 1, stats: statsFor({ ...source, uid: id }), statsVersion: 2 };
  ...
  function capture(sprite) { const active = party(), stored = repository(), instanceUid = makeUid(sprite.id), item = { ...sprite, uid: instanceUid, id: sprite.id, name: sprite.name, sprite: sprite.sprite, hp: Math.max(0, Number(sprite.hp ?? 100)), maxHp: Number(sprite.maxHp ?? 100), xp: 0, level: 1, stats: statsFor({ ...sprite, uid: instanceUid }), statsVersion: 2 }; const empty = slots().findIndex(entry => !entry); if (empty < 0) { stored.push(item); save(active, stored); window.dispatchEvent(new CustomEvent('databyte:party-updated')); return { ok: true, location: 'repository', items: active, repository: stored, uid: item.uid }; } const next = slots(); next[empty] = item; save(next, stored); window.dispatchEvent(new CustomEvent('databyte:party-updated')); return { ok: true, location: 'party', items: next.filter(Boolean), uid: item.uid }; }
