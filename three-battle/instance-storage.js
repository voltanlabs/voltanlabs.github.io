(function () {
  ...
  const normalizeInstance = item => { const next = { ...item }, declaredLevel = levelFor(next), minimumXp = 50 * (declaredLevel - 1) * declaredLevel; next.xp = Math.max(0, Number(next.xp) || 0, minimumXp); next.level = session.spriteProgress?.(next)?.level || declaredLevel; next.captureHp = Math.max(1, Math.min(hpCap(next), Number(next.captureHp) || Math.min(100, Number(next.maxHp) || 100))); next.maxHp = progressionFor(next).maxHp; next.hp = Math.min(next.maxHp, Math.max(0, Number(next.hp ?? next.maxHp))); next.statRolls = statRollsFor(next); next.stats = statsFor(next); next.statsVersion = 3; return next; };
  ...
  function capture(sprite) { const active = party(), stored = repository(), instanceUid = makeUid(sprite.id), level = levelFor(sprite), xp = Math.max(Number(sprite?.xp) || 0, 50 * (level - 1) * level), captureHp = Math.max(1, Number(sprite?.captureHp) || Math.min(100, Number(sprite?.maxHp) || 100)), currentHp = Math.max(0, Number(sprite?.hp ?? sprite?.maxHp ?? captureHp)), item = normalizeInstance({ ...sprite, uid: instanceUid, id: sprite.id, name: sprite.name, sprite: sprite.sprite, hp: currentHp, captureHp, maxHp: Number(sprite?.maxHp) || captureHp, xp, level, stats: { ...(sprite?.stats || {}) }, statRolls: sprite?.statRolls ? { ...sprite.statRolls } : undefined }); const empty = slots().findIndex(entry => !entry); if (empty < 0) { stored.push(item); save(active, stored); window.dispatchEvent(new CustomEvent('databyte:party-updated')); return { ok: true, location: 'repository', items: active, repository: stored, uid: item.uid }; } const next = slots(); next[empty] = item; save(next, stored); window.dispatchEvent(new CustomEvent('databyte:party-updated')); return { ok: true, location: 'party', items: next.filter(Boolean), uid: item.uid }; }
