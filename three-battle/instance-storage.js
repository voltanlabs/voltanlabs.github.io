(function () {
  ...
  const statsFor = item => { const configuration = item?.primaryConfiguration || item?.configurations?.[0] || item?.configuration || 'Default', base = statBases[configuration] || statBases.Default, stage = stageFor(item), stageBonus = (stage - 1) * 18, text = `${item?.id || ''}:${item?.uid || ''}`; let seed = 0; for (const char of text) seed = (seed * 31 + char.charCodeAt(0)) >>> 0; const rolls = item?.statRolls || { attack: base[0] + (seed % 13) - 6, defense: base[1] + ((seed >>> 4) % 13) - 6, speed: base[2] + ((seed >>> 8) % 13) - 6 }; const growth = (levelFor(item) - 1) * 0.85; return { attack: Math.max(1, Math.min(160, Math.round(rolls.attack + stageBonus + growth))), defense: Math.max(1, Math.min(160, Math.round(rolls.defense + stageBonus + growth))), speed: Math.max(1, Math.min(160, Math.round(rolls.speed + stageBonus + growth))), crit: Math.max(0, Math.min(50, Number(item?.stats?.crit ?? item?.crit ?? Math.floor(Math.random() * 51)))) }; };
