(function () {
  ...
  const hpFor = item => Math.min(hpCap(item), Math.max(1, Number(item?.captureHp) || 60) + Math.floor((hpCap(item) - Math.max(1, Number(item?.captureHp) || 60)) * (levelFor(item) - 1) / 99));
  const statsFor = item => { const configuration = item?.primaryConfiguration || item?.configurations?.[0] || item?.configuration || 'Default', base = statBases[configuration] || statBases.Default, stage = stageFor(item), stageBonus = (stage - 1) * 8, text = `${item?.id || ''}:${item?.uid || ''}`; let seed = 0; for (const char of text) seed = (seed * 31 + char.charCodeAt(0)) >>> 0; const rolls = item?.statRolls || { attack: base[0] + (seed % 13) - 6, defense: base[1] + ((seed >>> 4) % 13) - 6, speed: base[2] + ((seed >>> 8) % 13) - 6 }; const growth = (levelFor(item) - 1) * 0.4; return { attack: Math.max(1, Math.min(140, Math.round(rolls.attack + stageBonus + growth))), defense: Math.max(1, Math.min(140, Math.round(rolls.defense + stageBonus + growth))), speed: Math.max(1, Math.min(140, Math.round(rolls.speed + stageBonus + growth))), crit: Math.max(0, Math.min(50, Number(item?.stats?.crit ?? item?.crit ?? Math.floor(Math.random() * 51)))) }; };
  ...
  const createWildInstance = source => { const level = Math.max(1, Math.min(100, Number(source?.level) || 1)), captureHp = 60 + Math.floor(Math.random() * 41); return normalizeInstance({ ...source, uid: `wild-${source?.id || 'signal'}-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, captureHp, maxHp: captureHp, hp: captureHp, level }); };
  const normalizeInstance = item => { const next = { ...item }; next.captureHp = Math.max(1, Math.min(hpCap(next), Number(next.captureHp) || Math.min(100, Number(next.maxHp) || 100))); next.maxHp = progressionFor(next).maxHp; next.hp = Math.min(next.maxHp, Math.max(0, Number(next.hp ?? next.maxHp))); next.stats = statsFor(next); next.statRolls = next.statRolls || { attack: next.stats.attack, defense: next.stats.defense, speed: next.stats.speed }; next.statsVersion = 3; return next; };
