...
export function playerDamage({ move, attack = 60, defense = 60, attackerLevel = 1, defenderLevel = 1, attackMultiplier = 1, incomingMultiplier = 1, critical = false }) {
  ...
  const statRatio = Math.sqrt(Math.max(1, attack) / Math.max(1, defense));
  return Math.max(1, Math.round(Number(move.power || 0) * DAMAGE_PACE * multiplier * statRatio * levelMultiplier(attackerLevel, defenderLevel) * attackMultiplier * incomingMultiplier * (critical ? 1.5 : 1)));
}
...
export function enemyDamage({ raw, move = null, attack = 60, defense = 60, attackerLevel = 1, defenderLevel = 1, incomingMultiplier = 1, attackMultiplier = 1, guarding = false, leak = 0, critical = false }) {
  ...
  const statRatio = Math.sqrt(Math.max(1, attack) / Math.max(1, defense));
  return Math.max(1, Math.round((Number(raw) + Number(leak)) * DAMAGE_PACE * alignment * statRatio * levelMultiplier(attackerLevel, defenderLevel) * guardMultiplier * incomingMultiplier * attackMultiplier * (critical ? 1.5 : 1)));
}
