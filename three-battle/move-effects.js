(function () {
  ...
  function renderStatuses() {
    ...
    const badges = value => (Array.isArray(value) ? value : value ? [value] : []).map(status => { const description = descriptions[status.id] || 'A temporary signal effect.'; const label = `${status.id}${Number(status.stacks) > 1 ? ` x${status.stacks}` : ''} - ${status.duration}`; return `<span class="status-${positive.has(status.id) ? 'positive' : 'negative'}" data-status-id="${status.id}" tabindex="0" title="${description}" aria-label="${label}. ${description}" data-status-tip="${description}">${label}</span>`; }).join('');
