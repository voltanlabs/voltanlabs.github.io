(function () {
  ...
  function open() {
    ...
    modal.querySelectorAll('[data-map-region]').forEach(button => button.addEventListener('click', () => { window.DataByteSession?.profileSet?.('vl_three_battle_region', button.dataset.mapRegion); modal.classList.remove('is-open'); window.DataByteRegions?.render?.(); window.dispatchEvent(new CustomEvent('databyte:region-updated', { detail: window.DataByteRegions?.current?.() })); }));
