(function(){
  ...
  document.getElementById('captureBtn')?.addEventListener('click',e=>{const name=document.getElementById('enemyName')?.textContent?.trim().toLowerCase();const sprite=window.DataByteSession?.roster().find(item=>item.name.toLowerCase()===name);if(sprite){window.DataByteSession.capture({...sprite,sprite:'./data/sprites/'+sprite.sprite});e.currentTarget.textContent='SIGNAL CAPTURED';}else e.currentTarget.textContent='SIGNAL ESCAPED';e.currentTarget.disabled=true;const next=document.createElement('button');next.className='scan-button';next.textContent='CONTINUE SCANNING';next.addEventListener('click',returnToScanner);e.currentTarget.parentElement.appendChild(next);});
})();
